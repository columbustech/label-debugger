...
def run_with_analyst(params, history_prefix, num_files, correct_label_attr):
    ...
    all_detected_errors, iter_times = debug_labels_with_analyst(debugger, index2pair, pair2golden)
...
def debug_labels_with_analyst(debugger, index2pair, pair2golden):
    ...
    while True:
        ...
        # find their correct labels
        index2correct_label = { index:pair2golden[ (int(index2pair[index][0]), int(index2pair[index][1])) ]  for index in top_suspicious_indices}
        iter_count, num_errors, error_indices, det_error_poses  = debugger.analyze(index2correct_label)
